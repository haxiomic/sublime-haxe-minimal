# Generated by Haxe 4.0.0 (git build development @ 01cc41a)
# coding: utf-8
    ...
    def createServerMessageQueue(pipe):
        def _hx_local_1(pipe1,queue):
            ...
            while True:
                ...
                messageBytes = haxe_io_Bytes.ofData(pipe1.read(bytesRemaining))
                if ((messageBytes is None) or ((messageBytes.length != bytesRemaining))):
                    break
...
class StringTools:
    ...
    def htmlEscape(s,quotes = None):
        _this = s.split("&")
        _this1 = "&amp;".join([python_Boot.toString1(x1,'') for x1 in _this]).split("<")
        _this2 = "&lt;".join([python_Boot.toString1(x1,'') for x1 in _this1]).split(">")
        s = "&gt;".join([python_Boot.toString1(x1,'') for x1 in _this2])
        if quotes:
            _this3 = s.split("\"")
            _this4 = "&quot;".join([python_Boot.toString1(x1,'') for x1 in _this3]).split("'")
            return "&#039;".join([python_Boot.toString1(x1,'') for x1 in _this4])
        else:
            return s
